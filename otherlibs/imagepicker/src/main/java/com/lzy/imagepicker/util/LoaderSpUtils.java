...
public class LoaderSpUtils {

    public static final String NATION = "nation_sp";

    private SharedPreferences mSp;

    private static LoaderSpUtils instance;

    public LoaderSpUtils(Context ctx) {
        this(ctx, NATION);
    }

    public LoaderSpUtils(Context ctx, String file) {
        mSp = ctx.getSharedPreferences(file, Context.MODE_PRIVATE);
    }

    public static LoaderSpUtils getInstance(Context ctx) {
        if (instance == null) {
            instance = new LoaderSpUtils(ctx);
        }
        return instance;
    }
