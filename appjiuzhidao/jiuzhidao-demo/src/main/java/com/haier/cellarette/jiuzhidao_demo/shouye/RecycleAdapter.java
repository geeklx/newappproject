...
import com.haier.cellarette.baselibrary.zothers.SimpleTagImageView;
...
//        Glide.with(context).load(ratings.getSku_image()).skipMemoryCache(true).diskCacheStrategy(DiskCacheStrategy.NONE).into(viewHolder.iv_imgurl);
        ...
        if (ratings.isEnselect()){
            ...
            viewHolder.tv_content1.setTextColor(ContextCompat.getColor(context,R.color.orange_red));
        }else {
            //未选中
